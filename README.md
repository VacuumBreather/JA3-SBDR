...
- **Fix Squad Split:** Fixes and improves the item distribution that happens when squads are split. The logic in vanilla has a bug for ammo, so ammo is always evenly distributed instead of according to need. This mod fixes that and also improves parts distrubution (using highest MECH skill instead of number of mechanics per squad), and also auto-distributes all other craftables according to this skill check. Meds get distributed according to number of med kits (like in vanilla) however this mod adds the check for the new Reanimation Kit that was added in the DLC.
