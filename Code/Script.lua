...
function SquadBagDoneRight:MoveAllMercsInventoryToSquadBag()
	...
	for squad_id, squad in pairs(gv_Squads) do
		...
		if squad.Side == "player1" and squad.units and squad.squad_bag then
			local moved_items = {} -- [{ unit, slot_name, item, moved_amount }]
			local bag_copy = table.copy(squad.squad_bag) -- Create a copy to work on

			-- Verification: Sum of amounts before
			local amounts_before = {} -- class -> total_amount
			local function add_to_before(class, amount)
				amounts_before[class] = (amounts_before[class] or 0) + amount
			end

			-- Count in current bag
			...
			for _, unit_id in ipairs(squad.units) do
				...
				if unit then
					unit:ForEachItem(SquadBagItemClass, function(item, slot_name)
						...
						-- If anything remains, add it as a new item in the bag copy
						if remaining_amount > 0 then
							local new_item
			...
			-- Verification: Sum of amounts after
			local amounts_after = {} -- class -> total_amount
			...
			-- Count what's left in mercs (original amount - moved amount)
			for _, unit_id in ipairs(squad.units) do
				local unit = gv_UnitData and gv_UnitData[unit_id]
			...
			if verified then
				...
				-- 4. Only AFTER the bag is updated, remove the items from the units
				for _, move in ipairs(moved_items) do
					...
					-- Notify that item was added to bag (optional, but good for UI/feedback)
	...
	-- Refresh UI once if the currently opened bag was affected
